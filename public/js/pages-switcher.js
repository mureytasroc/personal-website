...
"use strict";
    ...
    function init(options) {
        ...
        $('body').append('<div id="page-ajax-loaded" class="page-ajax-loaded"></div>');
        ajaxLoader();
    }
    ...
    function ajaxLoader() {
        ...
        var href = $('.ajax-page-load').each(function(){
            ...
            if(location.hash == location.hash.split('/')[0] + href){
                ...
                var activeDataGroup = getActiveDataGroup();
                var prev = null;
                var next = null;
                var foundState = 0;
                $("#portfolio_grid .ajax-page-load").each(function(idx, el) {
                  var atThisElement = ($(el).attr('href') === href);
                  var dataGroups = JSON.parse($(el).parent().attr('data-groups'));
                  if (!atThisElement && dataGroups.includes(activeDataGroup)) {
                    if (foundState == 0) {
                      prev = $(el);
                ...
                ajaxLoadedContent.load(toLoad, function() {
                  var prevButton = $('#portfolio-previous-button');
                  var nextButton = $('#portfolio-next-button');
                  if (prev == null) { prevButton.hide(); } else {
                    prevButton.show();
                    prevButton.attr('href', prev.attr('href'));
                  }
                  if (next == null) { nextButton.hide(); } else {
                    nextButton.show();
        ...
        $(document)
            ...
            .on("click",".ajax-page-load", function () { // Show Ajax Loaded Page
                var hash = location.hash.split('/')[0] + $(this).attr('href');
                location.hash = hash;
